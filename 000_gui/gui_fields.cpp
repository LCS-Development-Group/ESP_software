...
gui_generic_field_t::gui_generic_field_t(gui_field_type_t _field_type, task_notify_pack_t *_ntpack, SemaphoreHandle_t _mutex)
...
gui_sw_field_t::gui_sw_field_t(
    ...
    uint16_t _pos_x,
    uint16_t _pos_y
):gui_generic_field_t(gui_field_type_t::SW_BOOL, _ntpack, _mutex), state(_state_var_ptr), text(_text), pos_x(_pos_x), pos_y(_pos_y)
{
...
gui_back_field_t::gui_back_field_t(lv_obj_t* screen)
:gui_generic_field_t(gui_field_type_t::BACK_BTN, nullptr, NULL)
{
    back_button=lv_label_create(screen);
